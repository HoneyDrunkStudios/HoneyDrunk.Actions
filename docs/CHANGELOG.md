...
- `job-dependency-scan.yml`: scan solution projects one project at a time with fail-closed `dotnet list package --vulnerable --include-transitive --format json` handling, derive the HoneyDrunk.Actions helper checkout ref from the called reusable workflow ref by default, verify the checked-out `.github/actions-repo` helper path before invocation, smoke-test the reusable workflow checkout/script/ref wiring, normalize nested solution project paths relative to the solution file, resolve `.` / `..` path segments during coverage validation, ignore SQL database projects that do not produce the `dotnet list package` JSON contract, and require callers to provide or auto-discover an actual solution/package-project target instead of treating the working directory as an implicit scan target. This is a stricter consumer-facing gate: repos with deep/non-standard layouts, multiple solutions, or project-level scan failures may newly fail and should pin a reviewed Actions ref/tag plus pass `project-path` explicitly before rollout.
