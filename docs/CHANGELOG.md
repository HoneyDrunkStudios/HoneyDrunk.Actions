...
- `nightly-security.yml`: granted `actions: read` to the reusable workflow and documented the caller permission baseline so CodeQL SAST can read workflow-run metadata while finalizing scheduled analysis. Without this scope, scheduled consumers can complete the build and SARIF export but fail the CodeQL analysis step with `Resource not accessible by integration`.
- `job-dependency-scan.yml`: scan solution projects one project at a time with fail-closed `dotnet list package --vulnerable --include-transitive --format json` handling, check out the HoneyDrunk.Actions helper path from the caller-controlled `actions-ref` input (`main` by default), verify the checked-out `.github/actions-repo` helper path before invocation, log the selected Actions ref and scan target, smoke-test the reusable workflow checkout/script/ref wiring including a real .NET SDK clean multi-project `.slnx`, normalize nested solution project paths relative to the solution file, resolve `.` / `..` path segments during coverage validation, ignore SQL database projects that do not produce the `dotnet list package` JSON contract, and require callers to provide or auto-discover an actual solution/package-project target instead of treating the working directory as an implicit scan target. This is a stricter consumer-facing gate: repos with deep/non-standard layouts, multiple solutions, or project-level scan failures may newly fail and should pin a reviewed Actions ref/tag plus pass `project-path` explicitly before rollout. Per-project scanning may increase CI minutes for larger solutions.
...
- `job-discord-notify.yml`: redaction runs twice against one shared pattern module — over the raw inputs and again over the final assembled JSON payload, so a JSON-escaped secret in `metadata` (e.g. `A`-encoded) cannot decode past the pre-check; the total Discord embed budget (6000 chars) is enforced across fields, not just the description (trailing metadata fields are dropped with an "(N fields omitted)" note rather than letting Discord 400 the whole alert); `link` is scanned by the pre-check; the `github_pat_` fine-grained PAT shape is covered; and `curl` has bounded `--connect-timeout`/`--max-time` with transport failures routed through the structured error path. The regex pre-check remains defense-in-depth against accidental plaintext leaks, not adversarial base64/gzip encoding (documented in the workflow header and README).

- `job-discord-notify.yml`: new tier-2 reusable workflow — the single CI-side seam for posting operator-alerts to Discord per ADR-0084 D9. Callable via `workflow_call` with inputs `channel` (one of the seven ADR-0084 D2 channels), `severity` (`info`/`medium`/`high`/`critical`), `title`, and optional `body`/`link`/`metadata` (JSON). Validates the channel/severity enums, runs a **fail-closed redaction pre-check** over the payload (GitHub tokens, JWTs, Discord webhook URLs, AWS keys, Azure connection-string fragments, PEM headers, keyword-adjacent high-entropy runs, card/SSN shapes) so no secret value, credential, or obvious PII reaches a channel (ADR-0084 D8 / Invariant 8 extended to webhook payloads — a match aborts the post rather than redact-and-continue), decorates by severity (embed color + emoji prefix), truncates title/body to Discord's embed caps, and POSTs the embed to the channel's `DISCORD_WEBHOOK_*` org secret via `curl` (Invariant 38 — no marketplace action wrapper). Declares `permissions: {}` (Invariant 39). Non-2xx responses fail the job with the response body redacted through the same pattern set. `DISCORD_WEBHOOK_AUDIT_SENSITIVE` is restricted to Architecture/Vault/Vault.Rotation/Audit per ADR-0084 D4. No consumers yet; the phased emitter rollout (ADR-0084 packets 10–13) wires callers. Ad-hoc `curl` to a Discord webhook URL outside this workflow is forbidden per ADR-0084 D11.
...
- `agent-run.yml`: added optional `work-item-path` input. When supplied, the workflow (1) injects a structured `> Work Item: <permalink>` instruction into the agent's prompt envelope and (2) runs a post-hoc "Assert PR-body work item link" step that mechanically inserts the canonical line into any PR the agent opened in the `checkout-target` repo. The workflow — not the LLM — is the mechanical guarantor of invariant 32 in HoneyDrunk.Architecture. The permalink resolves to the Architecture checkout's actual commit SHA (via `git rev-parse HEAD`) so the link is immutable, not a moving branch ref. Idempotent (no edit if the canonical line is already present) and soft on edge cases (no PR / no checkout-target / detached HEAD / main-branch run → notice + exit 0). Existing callers unaffected — `work-item-path` defaults to empty. Per ADR-0011 packet 03.
...
### Added
- `pr-core.yml`: ADR-0044 `authorship-check`, ADR-0011/ADR-0044 `pr-metadata-check`, and warnings-only `pr-size-check` jobs. Authorship now requires a parseable `Authorship:` PR-body line; agent/mixed PRs must declare work item metadata or an explicit out-of-band reason; non-`human` PRs get visible size discipline without blocking Phase 2 merges.
...
- `.github/pull_request_template.md`: local PR-body placeholders for ADR-0044 `Authorship:`, `Work Item:`, `Out-of-band reason:`, and `Size justification:` fields.
